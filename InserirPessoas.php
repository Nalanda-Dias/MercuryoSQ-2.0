<?php 
	...
	//Fazendo o upload dos curriculos
		// arquivo
    	...
    	preg_match("/\.(pdf){1}$/i",$arquivo['name'],$extencao);

    	$novoarquivo = md5(uniqid(time())).".".$extencao[1];
	...
	{ 
		...
		$Insert = $conexao->query("insert into tbl_TrabalheConosco (Nome, Sobrenome, Data_Nascimento, Nome_Estado_Civil, Email, Celular, Endereco, Numero, CEP, Nome_Estado, Cidade, Linguagens_Prog, Sistemas_Operac, Redes, Pacote_Office, Descricao_Formacao, Ultima_Empresa, Cargo, Data_Entrega, Data_Saida, Curriculo)
			values ('$Nome','$Sobrenome',str_to_date('$Data_Nascimento','%d/%m/%Y'),'$Estado_Civil','$Email','$Celular','$Endereco','$Numero','$CEP','$Estado','$Cidade','$Linguagens_Prog', '$Sistemas_Opera','$Redes','$Pacotes_Office','$Descricao_Formacao','$Ultima_Empresa','$Cargo',str_to_date('$Data_Entrada','%d/%m/%Y'),str_to_date('$Data_Saida','%d/%m/%Y'),'$novoarquivo');");

		header('location:sucessoTrabConosco.php');
